...
def _get_transformer_layer_spec(use_te, config):
    ...
    if use_te:
        ...
        return get_gpt_layer_local_spec(
            ...
            use_kitchen=False # config.use_kitchen,
        )
...
def loss_func(
    ...
    token_filter.ops.backward_filter(loss, (torch.rand_like(output_tensor) > 0.5).to(int))
    ...
    dot.graph_attr.update(
        ...
        pack='true',
        packmode='clust',
        label='DeepSpeed TinyLlama MoE + ZeRO1 Computation Graph',
        labelloc='top',
        fontcolor='blue'
    )
    ...
    filename = f"tmp/megatronlm_computation_graph_{torch.distributed.get_rank()}"
...
def forward_step(data_iterator, model: GPTModel, return_schedule_plan: bool = False):
    ...
    # if torch.distributed.get_rank() == 0:
    #     token_filter.ops.backward_filter(output_tensor, (torch.rand([1, 224]) > 0.5).to(int).to(output_tensor.device))
    #     dot = torchviz.make_dot(
    #         output_tensor, 
    #         params=dict(model.named_parameters()),
    #         show_saved=True,
